                 ...
                 double &reg)
{
  ...
  while(true)
  {
    ...
    double energy =
        libshell::ElasticShell<SFF>::elasticEnergy(mesh, curPos, curEdgeDOFs, mat, restState, &derivative, &hessian);
    ...
    if(solver.info() == Eigen::Success)
    {
      ...
      double newenergy =
          libshell::ElasticShell<SFF>::elasticEnergy(mesh, newPos, newEdgeDofs, mat, restState, &derivative, NULL);
      force = -derivative;
      ...
      if(newenergy <= energy)
      {
        std::cout << "Old energy: " << energy << " new energy: " << newenergy << " force residual " << forceResidual
                  << " pos change " << descentDir.segment(0, 3 * nverts).norm() << " theta change "
                  << descentDir.segment(3 * nverts, nedgedofs * nedges).norm() << " lambda " << reg << std::endl;
        curPos = newPos;
      ...
      {
        std::cout << "Not a descent direction; old energy: " << energy << " new energy: " << newenergy
                  << " lambda now: " << 2.0 * reg << std::endl;
      }
    }
    else
    {
