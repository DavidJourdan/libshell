...
template <class SFF>
double BilayerStVKMaterial<SFF>::stretchingEnergy(const MeshConnectivity &mesh,
  ...
  if(hessian)
  {
    ...
    *hessian += crossTermCoeff2 * dA2 * 0.5 * lameAlpha2_ * inner4.transpose() * inner2;
    ...
    for(int i = 0; i < 4; ++i) // iterate over Mainv and abarinv as if they were vectors
    {
      ...
      *hessian +=
          crossTermCoeff1 * dA1 * (0.5 * lameAlpha1_ * M1.trace() * abarinv1(i) + lameBeta1_ * Mainv1(i)) * augahess[i];
      *hessian +=
          crossTermCoeff2 * dA2 * (0.5 * lameAlpha2_ * M2.trace() * abarinv2(i) + lameBeta2_ * Mainv2(i)) * augahess[i];
      *hessian += crossTermCoeff1 * dA1 * (0.5 * lameAlpha1_ * sigma1.trace() * abarinv1(i) + lameBeta1_ * Sainv1(i)) *
                  bhess[i];
      *hessian += crossTermCoeff2 * dA2 * (0.5 * lameAlpha2_ * sigma2.trace() * abarinv2(i) + lameBeta2_ * Sainv2(i)) *
                  bhess[i];
    }
    ...
    *hessian +=
        crossTermCoeff1 * dA1 * lameBeta1_ * (inner011.transpose() * inner103 + inner103.transpose() * inner011);
    *hessian +=
        crossTermCoeff1 * dA1 * lameBeta1_ * (inner013.transpose() * inner101 + inner101.transpose() * inner013);
    ...
    *hessian +=
        crossTermCoeff2 * dA2 * lameBeta2_ * (inner012.transpose() * inner104 + inner104.transpose() * inner012);
    *hessian +=
        crossTermCoeff2 * dA2 * lameBeta2_ * (inner014.transpose() * inner102 + inner102.transpose() * inner014);
