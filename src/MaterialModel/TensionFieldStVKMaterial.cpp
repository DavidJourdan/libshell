...
template <class SFF>
double TensionFieldStVKMaterial<SFF>::stretchingEnergy(const MeshConnectivity &mesh,
                                                       ...
                                                       Eigen::Matrix<double, 9, 9> *hessian) const
{
  ...
  else
  {
    ...
    if(derivative || hessian)
    {
      ...
      Eigen::Matrix2d mat = 0.5 * abarinv + sign / denom *
                                                (T / 4.0 * abarinv -
                                                 1.0 / 2.0 * detAbarinv *
                                                     adjstrain); // lamda equals (tr(A) + sqrt(tr(A)^2 - 4 * det(A)))/2
      ...
      if(hessian)
      {
        ...
        //(*hessian) += dA * sign * lambda / denom * (-1.0 / 2.0 / abar.determinant()) * aderiv.row(3).transpose() *
        // aderiv.row(0);
        (*hessian) += 2.0 * kstretching * dA * sign * lambda / denom * (-1.0 / 2.0 * detAbarinv) *
                      aderiv.row(3).transpose() * aderiv.row(0);
        (*hessian) += 2.0 * kstretching * dA * sign * lambda / denom * (-1.0 / 2.0 * detAbarinv) * -1 *
                      aderiv.row(2).transpose() * aderiv.row(1);
        (*hessian) += 2.0 * kstretching * dA * sign * lambda / denom * (-1.0 / 2.0 * detAbarinv) * -1 *
                      aderiv.row(1).transpose() * aderiv.row(2);
        (*hessian) += 2.0 * kstretching * dA * sign * lambda / denom * (-1.0 / 2.0 * detAbarinv) *
                      aderiv.row(0).transpose() * aderiv.row(3);
...
double TensionFieldStVKMaterial<SFF>::bendingEnergy(
    ...
    const RestState &restState,
    int face,
    // F(face, i), then the three vertices opposite F(face,i), then the extra DOFs on oppositeEdge(face,i)
    Eigen::Matrix<double, 1, 18 + 3 * SFF::numExtraDOFs> *derivative,
