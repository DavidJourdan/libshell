                                        ...
                                        std::vector<Eigen::Triplet<double>> *hessian)
{
        ...
        for(int j = 0; j < 3; j++)
        {
          for(int k = 0; k < 3; k++)
          {
            for(int l = 0; l < 3; l++)
            {
              for(int m = 0; m < 3; m++)
              {
                hessian->push_back(Eigen::Triplet<double>(3 * mesh.faceVertex(i, j) + l, 3 * mesh.faceVertex(i, k) + m,
                                                          hess(3 * j + l, 3 * k + m)));
              }
            }
          }
  ...
  if(whichTerms & EnergyTerm::ET_BENDING)
  {
    ...
    for(int i = 0; i < nfaces; i++)
    {
      ...
      result +=
          mat.bendingEnergy(mesh, curPos, extraDOFs, restState, i, derivative ? &deriv : NULL, hessian ? &hess : NULL);
      if(derivative)
      {
        for(int j = 0; j < 3; j++)
        {
          ...
          for(int k = 0; k < 3; k++)
          {
            for(int l = 0; l < 3; l++)
            {
              for(int m = 0; m < 3; m++)
              {
                ...
                if(oppidxk != -1)
                  hessian->push_back(Eigen::Triplet<double>(3 * mesh.faceVertex(i, j) + l, 3 * oppidxk + m,
                                                            hess(3 * j + l, 9 + 3 * k + m)));
                int oppidxj = mesh.vertexOppositeFaceEdge(i, j);
                if(oppidxj != -1)
                  hessian->push_back(Eigen::Triplet<double>(3 * oppidxj + l, 3 * mesh.faceVertex(i, k) + m,
                                                            hess(9 + 3 * j + l, 3 * k + m)));
                if(oppidxj != -1 && oppidxk != -1)
                  hessian->push_back(
                      Eigen::Triplet<double>(3 * oppidxj + l, 3 * oppidxk + m, hess(9 + 3 * j + l, 9 + 3 * k + m)));
              }
            ...
            for(int m = 0; m < nedgedofs; m++)
            {
              ...
              {
                hessian->push_back(Eigen::Triplet<double>(3 * nverts + nedgedofs * mesh.faceEdge(i, j) + m,
